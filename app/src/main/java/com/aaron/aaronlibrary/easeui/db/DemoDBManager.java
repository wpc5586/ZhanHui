...
public class DemoDBManager {
    ...
    private DemoDBManager(){
        dbHelper = DbOpenHelper.getInstance(ZhanHuiApplication.getInstance().getApplicationContext());
    }
    ...
    synchronized public void saveContactList(List<EaseUser> contactList) {
        ...
        if (db.isOpen()) {
            ...
            for (EaseUser user : contactList) {
                ...
                if(user.getV_title() != null)
                    values.put(UserDao.COLUMN_NAME_USER_TITLE, user.getV_title());
    ...
    synchronized public Map<String, EaseUser> getContactList() {
        ...
        if (db.isOpen()) {
            ...
            while (cursor.moveToNext()) {
                ...
                String userTitle = cursor.getString(cursor.getColumnIndex(UserDao.COLUMN_NAME_USER_TITLE));
                EaseUser user = new EaseUser(username);
                user.setNick(nick);
                user.setAvatar(avatar);
                user.setUserId(userId);
                user.setV_title(userTitle);
     ...
     * @return
     */
    synchronized public Map<String, EaseUser> getContactListNe() {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Map<String, EaseUser> users = new Hashtable<String, EaseUser>();
        if (db.isOpen()) {
            Cursor cursor = db.rawQuery("select * from " + UserDao.TABLE_NAME /* + " desc" */, null);
    ...
    synchronized public void saveContact(EaseUser user){
        ...
        if(user.getAvatar() != null)
            values.put(UserDao.COLUMN_NAME_AVATAR, user.getAvatar());
        if(user.getUserId() != null)
            values.put(UserDao.COLUMN_NAME_USER_ID, user.getNick());
        if(user.getV_title() != null)
            values.put(UserDao.COLUMN_NAME_USER_TITLE, user.getNick());
        if(db.isOpen()){
            db.replace(UserDao.TABLE_NAME, null, values);
        }
    }
