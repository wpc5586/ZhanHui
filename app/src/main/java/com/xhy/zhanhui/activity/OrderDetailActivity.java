...
import com.xhy.zhanhui.http.vo.OrderStateVo;
...
//        });
     ...
     */
    private void cancel() {
        PostCall.put(mContext, ServerUrl.cancelReservation(data.getReservation_id(), "5"), "state=", new PostCall.PostResponse<BaseBean>() {
            @Override
            public void onSuccess(int statusCode, byte[] responseBody, BaseBean bean) {
                showToast("取消成功");
                setResult(RESULT_OK);
