...
import com.xhy.zhanhui.http.domain.ExhibitionCompanyListBean;
...
public class CompanyListActivity extends ZhanHuiActivity {
    ...
    protected void init() {
        ...
        if (getIntent().hasExtra("hall_name"))
            hallName = getIntent().getStringExtra("hall_name");
        setActionbarTitle(hallName);
        getData();
    }

    private void getData() {
        PostCall.get(mContext, ServerUrl.halls(hallId), new BaseMap(), new PostCall.PostResponse<ExhibitionCompanyListBean>() {
            @Override
            public void onSuccess(int statusCode, byte[] responseBody, ExhibitionCompanyListBean bean) {
                List<ExhibitionCompanyListBean.Obj> dataBean = bean.getData();
                setRecyclerView();
                ...
                for (int i = 0; i < size; i++) {
                    List<ExhibitionCompanyListBean.Obj> tempDatas = new ArrayList<>();
                    int pos = 3 * i;
        ...
        }, new String[]{}, false, ExhibitionCompanyListBean.class);
    }
    ...
    public class ExhibitionDetailAdapter extends RecyclerView.Adapter<ExhibitionDetailHolder> implements View.OnClickListener, View.OnLongClickListener {
        ...
        public void onBindViewHolder(ExhibitionDetailHolder holder, int position) {
            ...
            if (data instanceof String) {
                ...
                for (int i = 0; i < size; i++) {
                    ExhibitionCompanyListBean.Obj tempData = (ExhibitionCompanyListBean.Obj) ((List) data).get(i);
