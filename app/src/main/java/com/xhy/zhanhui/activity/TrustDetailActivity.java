...
//            ((LinearLayout.LayoutParams) rlBottom.getLayoutParams()).topMargin = MathUtils.dip2px(mContext, 15);
    ...
    private void accept() {
        ...
        EMClient.getInstance().contactManager().asyncAcceptInvitation(data.getHx_username(), new EMCallBack() {
            ...
            public void onSuccess() {
                PostCall.putJson(mContext, ServerUrl.acceptFriend(), new ReceiveTrustVo(data.getHx_username(), getHxUserId()), new PostCall.PostResponse<BaseBean>() {
                    @Override
    ...
    private void refuse() {
        ...
        EMClient.getInstance().contactManager().asyncDeclineInvitation(data.getHx_username(), new EMCallBack() {
            @Override
            public void onSuccess() {
                PostCall.putJson(mContext, ServerUrl.declineFriend(), new ReceiveTrustVo(data.getHx_username(), getHxUserId()), new PostCall.PostResponse<BaseBean>() {
                    @Override
                    public void onSuccess(int statusCode, byte[] responseBody, BaseBean bean) {
                        dismissProgressDialog();
                        showToast("拒绝成功");
