...
import com.xhy.zhanhui.http.domain.VcardResultBean;
...
//                etWebsite.setText("www.baidu.com");
    ...
    private void save() {
        ...
        PostCall.postJson(mContext, ServerUrl.users(), vo, new PostCall.PostResponse<VcardResultBean>() {
            @Override
            public void onSuccess(int statusCode, byte[] responseBody, VcardResultBean bean) {
                showToast("保存成功");
                finish();
                if (bean.getData() != null && !TextUtils.isEmpty(bean.getData().getVcard_id()))
                    ZhanHuiApplication.getInstance().updateVcardId(bean.getData().getVcard_id());
        ...
        }, new String[]{}, true, VcardResultBean.class);
    }
