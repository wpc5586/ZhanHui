...
import com.xhy.zhanhui.http.domain.ExhibitionCompanyBean;
...
public class CompanyFragment extends ZhanHuiFragment implements OnItemClickListener {
...
//                .stopTurning();    //关闭自动切换
    ...
    private void getData() {
        PostCall.get(mContext, ServerUrl.exposCompany(), new BaseMap(), new PostCall.PostResponse<ExhibitionCompanyBean>() {
            @Override
            public void onSuccess(int statusCode, byte[] responseBody, ExhibitionCompanyBean bean) {
                if (ptrFrameLayout.isRefreshing())
                    ptrFrameLayout.refreshComplete();
                List<ExhibitionCompanyBean.Obj> dataBean = bean.getData();
                setRecyclerView();
                for (int i = 0; i < dataBean.size(); i++) {
                    ExhibitionCompanyBean.Obj tempData = dataBean.get(i);
                    adapter.addData(tempData.getHall_name() + "hallId" + tempData.getHall_id());
                    ...
                    for (int j = 0; j < size; j++) {
                        ...
                        for (int k = pos; k < pos + 3; k++) {
                            if (k < tempData.getCompanys().size()) {
                                tempDatas.add(tempData.getCompanys().get(k));
                            }
                        }
        ...
        }, new String[]{}, false, ExhibitionCompanyBean.class);
    }
